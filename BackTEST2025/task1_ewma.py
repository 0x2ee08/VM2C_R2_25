...
# ------------------------------------------------------------
mu_weekly_pct_ewma = returns_df.ewm(span=52, adjust=False).mean().iloc[-1]

# Regular std deviation (non-weighted) for sigma
σ_weekly_pct = returns_df.std(ddof=0, skipna=True)

# Convert to annualized
mu_annual_pct = mu_weekly_pct_ewma * ANNUAL_FACTOR
