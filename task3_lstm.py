...
def predict(past: pd.DataFrame, 
            window_size: int = 50, 
            epochs: int = 20, 
            batch_size: int = 1) -> np.ndarray:
    """
    Train an LSTM model per stock column to predict the next percentage change.
    
    Parameters:
    - past: DataFrame of shape (n_samples, n_stocks) with percentage changes.
    - window_size: Number of past time steps to use for each prediction.
    - epochs: Training epochs for each model.
    - batch_size: Batch size for model training.
    
    Returns:
    - predicted_pct: 1D numpy array of length n_stocks with next-step predictions.
    """
    predicted_pct = []
    for col in past.columns:
        series = past[col].values.reshape(-1, 1)
        ...
        predicted_pct.append(pred)
    
    return np.array(predicted_pct)
